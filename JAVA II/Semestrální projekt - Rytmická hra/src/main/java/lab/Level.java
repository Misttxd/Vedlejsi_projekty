...
public class Level {
    ...
    // private double debugTimer = 0; // bylo pro testovani


    public Level(double width, double height, Difficulty difficulty, MapInfo map, MediaPlayer mediaPlayer, GameSettings gameSettings) {
        ...
        noteSorter = new Comparator<DrawableSimulable>() {
            @Override
            public int compare(DrawableSimulable o1, DrawableSimulable o2) {
                if (o1 instanceof WorldEntity && o2 instanceof WorldEntity) {
                    WorldEntity e1 = (WorldEntity) o1;
                    WorldEntity e2 = (WorldEntity) o2;
    ...
    public void checkRelease(int lane) {
        ...
        Note releaseNote = findWaitingReleaseNote(lane);

        if (releaseNote != null) {
            Note note = releaseNote;
            double arrowY = note.getHeadY() - 40 - 10;
            double toleranceUp = 30;
        ...
        Note heldNote = findHeldNote(lane);
    ...
    public void draw(GraphicsContext gc) {
        ...
        for (int i = 0; i < lanecount; i++) {
            ...
            if (lanePressed[i] == 1) { // Úspěšný zásah -> barevná dráha               
                gc.setFill(baseColor.deriveColor(0, 1, 1.5, 0.7));
            } else if (lanePressed[i] == 2) { // Chybný stisk -> červená
                gc.setFill(Color.RED.deriveColor(0, 1, 1, 0.5));
            } else { // Normální stav -> jemná barva dráhy
    ...
    private void updateLaneColors() {
        // Drahy zustavaji rozsvicene dokud hrac nepusti klavesu (reset v checkRelease)
        // Pouze aktualizujeme na 1 pokud drzime dlouhou notu
        for (int i = 0; i < lanecount; i++) {
            int lane = i;

            // Kontrola, jestli je v dane draze drzena dlouha nota
            if (hasHeldNote(lane)) {
                lanePressed[lane] = 1;
            }
        }
    ...
    public void checkHit(int lane) {
        // skryje napovedu pro danou lane po prvotnim zmacknuti
        showKeyHints[lane] = false;

        // Kontrola, jestli uz drzime dlouhou notu
        boolean alreadyHolding = hasHeldNote(lane);

        if (alreadyHolding) {
            return;
        }

        Note hitNote = findHitNote(lane);
    ...
    public void longNoteCompleted() {
        //puvodne se pridal jeste jednou overdrive
    }
    ...
    private Note findWaitingReleaseNote(int lane) {
        for (DrawableSimulable entity : entities) {
            if (entity instanceof Note) {
                Note note = (Note) entity;
                if (note.getLane() == lane && note.isWaitingForRelease()) {
                    return note;
                }
            }
        }
    ...
    private Note findHeldNote(int lane) {
        for (DrawableSimulable entity : entities) {
            if (entity instanceof Note) {
                Note note = (Note) entity;
                if (note.getLane() == lane && note.isBeingHeld()) {
                    return note;
                }
            }
        }
    ...
    private boolean hasHeldNote(int lane) {
        return findHeldNote(lane) != null;
    }

    private Note findHitNote(int lane) {
        for (DrawableSimulable entity : entities) {
            if (entity instanceof Note) {
                Note note = (Note) entity;
                if (note.getLane() == lane && note.isActive() && !note.isBeingHeld() && !note.wasHit() && note.isAnyPartInHitZone()) {
                    return note;
                }
            }
        }

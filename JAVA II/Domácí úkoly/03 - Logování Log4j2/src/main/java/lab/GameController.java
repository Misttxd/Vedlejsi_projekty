...
public class GameController {
    ...
    public void startGame(MapInfo map) {
        log.info("Starting game with map: {} ({})", map.getMapName(), map.getDifficulty());
        this.currentMap = map;
                ...
                javafx.application.Platform.runLater(() -> {
                    ...
                    if (result.isPresent() && !name.isEmpty()) {
                        ...
                        try (BufferedReader in = new BufferedReader(
                                new InputStreamReader(new URI(url).toURL().openStream()))) {
                            ...
                            log.error("Error communicating with faculty website", e);
                        }
                    }
                    ...
                    try {
                        ...
                        log.error("Failed to return to menu after game", e);
                    }
                });
            }
        });
    ...
    private void togglePause() {
        isPaused = !isPaused;
        log.debug("Game paused state toggled to: {}", isPaused);
    ...
    public void onMenuClicked(ActionEvent event) throws Exception {
        log.debug("Returning to menu from game pause menu.");
        stop();
        app.returnToMenu();
    }
    ...
    public void onSettingsClicked(ActionEvent event) {
        try {
            ...
            log.error("Failed to open settings from game", e);
            e.printStackTrace();
        }
    }

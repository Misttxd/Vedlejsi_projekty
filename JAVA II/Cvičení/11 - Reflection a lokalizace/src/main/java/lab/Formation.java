...
public abstract class Formation<T extends DrawableSimulable> extends WorldEntity implements Collisionable {
    ...
    private final MyPoint speed = new MyPoint(20, 0);

    protected Formation(World world, MyPoint position, T... entities) {
    ...
    public Rectangle2D getBoundingBox() {
        return entitiesInFormation.stream().map(DrawableSimulable::getBoundingBox)
            .reduce(new Rectangle2D(position.getX(), position.getY(), 0, 0), (rect1, rect2) -> {
    ...
    public List<Collisionable> getCollisionableEntities() {
        return entitiesInFormation.stream().filter(Collisionable.class::isInstance).map(Collisionable.class::cast)
            .toList();
    }
    ...
    public void hitBy(Collisionable another) {
        if (another instanceof Formation<? extends DrawableSimulable> anotherFormation) {
            anotherFormation.getCollisionableEntities().forEach(
                anotherEntity -> getCollisionableEntities().stream().filter(c -> c.intersect(anotherEntity))
                    .forEach(c -> c.hitBy(anotherEntity)));
        } else {

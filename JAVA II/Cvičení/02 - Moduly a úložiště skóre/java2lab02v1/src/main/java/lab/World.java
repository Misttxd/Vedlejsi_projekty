...
public class World {
	...
	public World(double width, double height) {
        ...
        entities.add(cannon);
        entities.addAll(Stream.generate(() -> new Ufo(this)).limit(5).toList());

        entitiesToAdd.add(new RotatingUfoFormation(this, new Point2D(100, 200),
            new Ufo(this),
            new Ufo(this),
            new Ufo(this),
            new Ufo(this),
	...
	public void simulate(double deltaTime) {
        ...
        for (DrawableSimulable e2 : entitiesToRemove) {
            if(!entities.remove(e2)) {
                for(Formation<? extends DrawableSimulable> formation : entities.stream().filter(e -> e instanceof Formation<? extends DrawableSimulable>).map(Formation.class::cast).toList()){

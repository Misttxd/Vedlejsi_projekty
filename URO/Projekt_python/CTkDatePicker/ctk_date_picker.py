...
class CTkDatePicker(ctk.CTkFrame):
    ...
    def set_date(self, date=None, today=False):
        """
        Set the date in the date entry field.

        Parameters:
        - date (datetime, optional): A datetime object to set as the date.
        - today (bool): If True, sets the date to today's date.

        Sets the date entry field to the specified date or today's date.
        """
        if today:
            date = datetime.now()
        if date is None:
